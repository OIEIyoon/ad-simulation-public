...
| | Gigaflow | GPUDrive | PufferDrive |
|---|---|---|---|
| 병렬 환경 수 | **38,400** | 512+ | 256 (16×16) |
| 환경당 에이전트 | 최대 150 | 수십 (시나리오 의존) | **최대 1,024** |
| 총 동시 에이전트 | **~576만** | ~수만 | ~262K |
...
| 용어 | 의미 | 설명 |
|------|------|------|
| **SPS** | Steps Per Second | 초당 시뮬레이션 스텝 수 (end-to-end, 학습 포함) |
| **ASPS** | Agent Steps Per Second | 초당 에이전트별 스텝 수 (raw 시뮬레이션만) |
| **CASPS** | Controlled Agent SPS | 초당 **제어 대상** 에이전트 스텝 수 |

예: 100 환경 × 10 에이전트 × 100 스텝/초 = 100K ASPS

### 4.2 성능 비교

| | Gigaflow | GPUDrive | PufferDrive |
|---|---|---|---|
| Raw ASPS | **~120만** | **230만** (peak) | - |
| CASPS | - | ~200K | - |
| End-to-end SPS | **~120만** | ~50K | **~320K** |
| 총 학습량 | **1조 transition (1.6B km)** | ~수천만 steps | ~수억 steps |
| 학습 시간 | 10일 (8×A100) | 15시간/1K 시나리오 | ~15분/10K 시나리오 |
| 비용 | ~$5/100만km 주행 | <2 GPU-days | 단일 GPU |
...
| 기법 | 설명 | 효과 |
|------|------|------|
| **BVH** (Bounding Volume Hierarchy) | 계층적 공간 분할로 충돌 후보 쌍 축소 | O(N²) → O(N log N) |
...
**Gigaflow의 Frenet 좌표**: "차선 중심에서 0.7m 벗어남, 차선 대비 3° 틀어짐" → 맵 무관하게 의미 동일, 일반화 유리
...
| | Gigaflow | GPUDrive | PufferDrive |
|---|---|---|---|
| 알고리즘 | **Spatial Hash + Swept-Volume** | **BVH** (Bounding Volume Hierarchy) | **Box2D** |
...
Gigaflow의 swept-volume: Δt=0.3초에 고속 이동 시 프레임 사이 관통(tunneling) 방지. GPUDrive는 Δt가 작아(0.1초) tunneling 위험이 상대적으로 낮음.
...
| 파라미터 | 값/범위 |
|---------|--------|
| $C_{throttle}$ | $\sim X(1.25)$, 혼합 균일분포 |
| $C_{steer}$ | $\sim X(1.25)$ |
| $C_{acc}$ | $\sim X(1.5)$ |
...
혼합 균일분포 $X(a) = 0.5 \cdot U(a^{-1}, 1) + 0.5 \cdot U(1, a)$: 1 기준 대칭 분포 생성
...
| | Gigaflow | GPUDrive | PufferDrive |
|---|---|---|---|
| **에피소드 길이** | **1200 steps = 360초** | 91 steps = 9.1초 | 91 steps = 9.1초 |
| **Δt** | 0.3초 | 0.1초 | 0.1초 |
| **실시간 환산** | **6분** | **~9초** | **~9초** |
...
| **Goal 도달률** | - | **95%** (1K 시나리오) |
| **강건성** | **17.5년 / 300만km per incident** | 미보고 |
